...
export const onFileChange = async (context) => {
  ...
  if (!fs.existsSync(claudePath) && isCodeDirectory(dir)) {
...
export const scanProjectDocumentation = async (rootPath = process.cwd()) => {
  ...
  const scanDirectory = (dirPath, depth = 0) => {
    ...
    try {
      const entries = fs.readdirSync(dirPath, { withFileTypes: true });
      const hasCode = isCodeDirectory(dirPath);

      // Check for BRIEF.md
      const briefPath = path.join(dirPath, 'BRIEF.md');
      if (!fs.existsSync(briefPath) && depth > 0) {
        issues.push({
          type: 'MISSING_BRIEF',
          path: dirPath,
          severity: 'warning'
      ...
      if (hasCode && !fs.existsSync(claudePath)) {
        issues.push({
          type: 'MISSING_CLAUDE',
          path: dirPath,
          severity: 'error'
      ...
      for (const entry of entries) {
        if (entry.isDirectory()) {
          scanDirectory(path.join(dirPath, entry.name), depth + 1);
        }
      }
    } catch (error) {
...
`;
      ...
      fs.writeFileSync(claudePath, claudeTemplate);
