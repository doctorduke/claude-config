You are evaluating a Product Requirements Document. Your job determine if an engineering team can build this without needing a clarifying meetings.

Evaluate on these zones (0-5):

1. COMPLETENESS
- Are acceptance criteria specific and testable? (Not "should be fast" but "P99 is <300ms load")
- Are edge cases and failure modes identified?
- Are non-goals explicit? (What we deliberately won't do)

Score 5: All are measurable, edge cases documented, non-goals prevent scope creep
Score 3: AC present but vague, missing edge cases
Score 0: No AC or they're untestable ("improve UX")


2. TESTABILITY
- Can we write test cases directly from the AC?
...
Score 3: Rationale implied but not explicit, trade-offs are missing
  ...
  "revisit": [
    {
      ...
      "user_story": {"present": true, "quality": "good"},
      "acceptance_criteria": {"present": true, "quality": "vague"},
      "success_metrics": {"present": false},
      "non_goals": {"present": true, "quality": "good"}
    }
  ],
  "critical_gaps": [
    "No success metrics - how will we measure if this achieves the goal?",
    "Acceptance criteria are vague terms: 'performant', 'intuitive', 'responsive'"
  ],
  "qa_flags": [],
  "key_fixes": [
    {
      ...
      "problem": "'should be fast and responsive' is not testable",
      "fix": "Replace with 'P95 API response time <300ms under 1000 RPS load measured via loadbng'",
      "why": "QA needs measurable thresholds"
    },
    {
      "priority": 2,
      "location": "Missing section",
      "problem": "Missing success metrics",
      "fix": "Add section 'Success Metrics' like '10% reduction in support ticket volume related to payment flow within 4 weeks of launch. (2) Payment completion rate improves from 82% to 87%.'",
    ...
    },
    {
      ...
      "fix": "Specify Stripe API version (eg v3), auth (is this a restricted key use on payment intents_restricted?), rate limit 100 requests/second.",
      "why": "Engineering needs this to design the integration correctly"
    }
  ],
  "questions_eng_will_ask": [
    "What happens if the external API is down? Do we queue, fail gracefully, or retry?",
    "Is there a rate limit on the vendor API? Do we need request throttling?",
    "Who approves refunds >$1000? Is there an approval workflow or manual override here?"
  ]
}

Thresholds:
- ACCEPT: 4.2+ overall, all required elements present with good quality, <2 critical gaps
- REVISE: 3.0-4.1 overall, or missing 1 required element, or 3+ critical gaps
- REJECT: <3.0 overall, or missing 2+ required elements, or fundamentally unclear what's being built
